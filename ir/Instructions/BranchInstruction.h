...
public:
    ///
    /// @brief 构造函数（双向条件跳转 bc）
    /// @param _func 所属函数
    /// @param _op 操作符（IRINST_OP_BC）
    /// @param _condVar 条件变量（类型为i1）
    /// @param _trueLabel 条件为真时的跳转目标
    /// @param _falseLabel 条件为假时的跳转目标
    ///
    ...
    ///
    /// @brief 获取真跳转目标
    /// @return LabelInstruction* 真标签（仅适用于BC指令）
    ///
    [[nodiscard]] Instruction * getTrueTarget() const;

    ///
    /// @brief 获取假跳转目标
    /// @return LabelInstruction* 假标签（仅适用于BC指令）
    ///
    [[nodiscard]] Instruction * getFalseTarget() const;
...
private:
    ...
    Instruction * trueTarget;  ///< 真跳转目标（仅用于BC指令）
    Instruction * falseTarget; ///< 假跳转目标（仅用于BC指令）
